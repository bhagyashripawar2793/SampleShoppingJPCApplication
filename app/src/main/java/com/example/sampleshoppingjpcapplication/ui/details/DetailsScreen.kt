...
fun StackScope.Body(place: Place) {
    ...
    VerticalScroller(modifier = Modifier.gravity(Alignment.TopCenter)) {
        ...
        )
        {
            ...
            Image(
                imageResource(id = place.imageResources[0]),
                contentScale = ContentScale.Crop,
                modifier = Modifier.preferredHeight(230.dp).fillMaxWidth().tag("image")
            )
            ...
            TabRow(modifier = Modifier.tag("tab"),
                items = sectionTitles,
                selectedIndex = section.ordinal,
                indicatorContainer = IndicatorContainer(section),
            ...
            Container(
                place,
                modifier = Modifier.padding(start = 16.dp, top = 16.dp, end = 16.dp, bottom = 70.dp)
                    .tag("container"),
                section = section
            )
